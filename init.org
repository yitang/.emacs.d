...
# Firstly, install all the packages that are used in this configuration
# via Emacs's package management tool. It may takes a while if there are
# many packages are not installed in the /~/.emacs.d/elpa/ folder. 

# Firstly, all the packages used in this configuration are needed to be
# installed in the /~/.emacs.d.elpa// folder. It is recommended to install
# packages via Emacs's package management tool. 

# Note if there are many are not installed, it may takes a while. 
...
*** My settings based on Bernt's 
[2015-07-20 Mon 13:11]
