...
define the markups - no longer needed. those are built-in to org-mode.
#+begin_src emacs-lisp :results silent :tangle no
...
Jeklly have few conventions, like the new, YAML etc, it is better to
solve it automatically and in Emacs, as part of the export process.
...
I can quickly scan the folder to see how many outstanding drasfts and
which posts is published in cease I need to edit it. I bind it to =C-c
j p= and =C-c j d= where /j/ means Jelly, /p/ means posts, and /d/
means drafts.
...
(setq org-publish-project-alist
	...
	("myblog_jekyll_html"
         ...
         :publishing-directory "~/matrix/learning/mywebsite/blog"
         :recursive t
         :publishing-function jekyll-html-publish-to-html
	...
	;; ("myblog_jekyll_md"
        ...
        ;;  :publishing-directory "~/matrix/learning/mywebsite/blog"
        ;;  :recursive t
        ;;  :publishing-function jekyll-md-publish-to-md
        ;;  :with-toc nil
        ;;  :headline-levels 4
        ;;  :section-numbers nil
        ;;  :auto-preamble nil
...
(defun yt/jekyll--wrap-src-block (data backend info)
  (cond
   ((eq 'html backend)
    (message "wrap-src-block function")
    (message "%s" data)
    )
   data
   ))

(defun yt/jekyll--before-parsing (backend)
  (cond
   ((eq 'html backend)
...
(defun yt/jekyll--before-processing (backend)
  (cond
   ((eq 'html backend)
...
(add-hook 'org-export-before-parsing-hook 'yt/jekyll--before-parsing)
(add-hook 'org-export-before-parsing-functions 'yt/jekyll--before-processing)

(add-to-list 'org-export-filter-src-block-functions 'yt/jekyll--wrap-src-block)
#+end_src
