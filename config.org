...
** NEXT update CUSTOM_ID property
:PROPERTIES:
...
(defun yt/jekyll--create-or-update-custom_id-field ()
  "so that the CUSTOMER_ID propert is aligned with the headline, and
therfore the URL reflects to the headline. otherwise, the URL
will be a random number."
  (org-entry-put nil "CUSTOM_ID" (org-entry-get nil "ITEM"))
  )

(add-hook 'org-export-before-processing-hook 'yt/jekyll--create-or-update-custom_id-field-buffer)
#+end_src
