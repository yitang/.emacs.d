...
(defvar jekyll-publish-dir "~/matrix/learning/mywebsite/blog/_posts"
  "Relative path to posts directory.")
(defvar jekyll-assets-dir "~/matrix/learning/mywebsite/blog/assets/"
...
I can quickly scan the folder to see how many outstanding drafts and
which posts is published in cease I need to edit it.
...
contents and others etc. This is the template I am using.

The =jekly-ymal-escape= function is used just to handle the quote
and : characters in the title field.
...
(defun yt/jekyll--export-to-md ()
  ...
  (let* ((org-name (file-name-nondirectory (file-name-sans-extension buffer-file-name)))
	 (post-name (concat jekyll-directory jekyll-posts-dir org-name ".md")))
...
(defun yt/jekyll--export-to-html ()
  (interactive)
  (message "not implemented. use markdown instead.")
...
(defun yt/jekyll-insert-image (src caption)
  (interactive (list (read-file-name "images to include: " jekyll-assets-dir)
		     (read-string "Caption: ")))
  (insert (format jekyll-insert-image-liquid-template src caption))
  )
#+end_src
...
#+begin_src emacs-lisp :results silent :tangle no
(format "%% inlcude image.html src=%s" "aaa.png")
#+end_src
...
(defun yt/compile (buffer-name cmd &optional working-directory)
  ...
  (with-current-buffer "*compilation*"
    (rename-buffer (concat "*Compilation*" buffer-name))
    )
  )
#+end_src
** ibuffer - 
