...
#+begin_src elisp :var search-tag="SFG" search-file='("~/git/org/contacts.org")
(setq h (org-map-entries (lambda ()
                           (let* ((address (org-entry-get nil "EMAIL"))
                                  (name (elt (org-heading-components) 4)))
                             (cons name address)))
                         search-tag search-file))
...
#+call: GetContactGroup()

#+results:
: Ross Towe <Ross.Towe@jbaconsulting.com>, Zora van Leeuwen <Zora.VanLeeuwen@jbaconsulting.com>


#+call: GetContactGroup("JBA&SFG")
...
#+call: GetContactGroup("Friends&Uni")
